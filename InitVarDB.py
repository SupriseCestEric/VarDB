...
conn = mysql.connector.connect(host='localhost',user='##',password='##',database='CAP_database')
cur = conn.cursor()
...
cur.execute('''DROP TABLE IF EXISTS HGVS''')
...
cur.execute('''CREATE TABLE Transcripts (id   INTEGER NOT NULL AUTO_INCREMENT, name    VARCHAR(255), UNIQUE (id), UNIQUE (name), PRIMARY KEY (id))''')
...
cur.execute('''CREATE TABLE HGVS (
    ...
    UNIQUE (id),
    PRIMARY KEY (id),
    UNIQUE INDEX transcript_HGVSc (transcript, HGVSc)
) ENGINE INNODB''')
