...
csv_reader = reader(open("HD827viz.1.names","r"), quotechar="\"")
...
for item in list_files:
    subprocess.call(["mkdir", "/mnt/data/DB/HD827_DB/outfiles"])
    query1 = "https://10.111.243.18:443/api/v1/analysis?format=json&name=%s&start_date=%s&end_date=%s" %(item[0], then.date(), now.date())
    query2 = "Content-Type:application/x-www-form-urlencoded"
    auth = "Authorization:##"
    out = "/mnt/data/DB/HD827_DB/outfiles/%s.zip" %item[0]
    outdir = "/mnt/data/DB/HD827_DB/outfiles/%s" %item[0]
    ...
    response = requests.get('https://10.111.243.18:443/api/v1/analysis', headers=headers, params=params, verify=False)
    ...
    subprocess.call(["rm", "-rf", "/mnt/data/DB/HD827_DB/outfiles"])
    subprocess.call(["python3", "Add2VarDB.py", "-i", vcf])
    subprocess.call(["rm", vcf])

open("HD827viz.1.names","w").close()
